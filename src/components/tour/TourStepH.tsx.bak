...
import { CompareObjects } from "@/components/interactive/CompareObjects";
...
export function TourStepH({ onNext }: TourStepProps) {
  const level = getLevel("l3");
  const zones = getZonesByLevel("l3");
  const featuredObjects = getObjectsByLevel("l3").slice(0, 4);
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {featuredObjects.map((obj) => (
                  <Link
                    ...
                    className="group rounded-lg border border-white/10 bg-white/5 p-4 transition hover:border-violet-400/50 hover:bg-violet-500/10"
                  >
