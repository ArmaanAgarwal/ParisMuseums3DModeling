...
import { MeasurementQuiz } from "@/components/interactive/MeasurementQuiz";
import { DataOverlay } from "@/components/interactive/DataOverlay";
...
export function TourStepF({ onNext }: TourStepProps) {
  const level = getLevel("l2");
  const zones = getZonesByLevel("l2");
  const featuredObjects = getObjectsByLevel("l2").slice(0, 4);
