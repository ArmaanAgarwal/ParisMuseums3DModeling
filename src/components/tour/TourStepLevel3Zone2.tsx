...
interface TourStepLevel3Zone2Props {
  onNext: () => void;
  onPrevious: () => void;
}

export function TourStepLevel3Zone2({ onNext, onPrevious }: TourStepLevel3Zone2Props) {
  const zones = getZonesByLevel("l3");
  const secondZone = zones[1];
  const exhibits = secondZone ? getExhibitsByZone(secondZone.id) : [];
  const objects = exhibits.flatMap((ex) => getObjectsByExhibit(ex.id));
  const featuredObjects = objects.slice(0, 4);

  if (!secondZone) return null;
        ...
        >
          <div className="mx-auto max-w-4xl">
            ...
            <p className="mb-6 text-xl leading-relaxed text-white/80">
              {secondZone.zoneIntent || secondZone.summary}
            </p>
          </div>

          {/* What to Notice */}
          {secondZone.whatToNotice && secondZone.whatToNotice.length > 0 && (
            <div className="mx-auto max-w-4xl rounded-2xl border border-white/15 bg-white/5 p-8">
              ...
              <ul className="space-y-3">
                {secondZone.whatToNotice.map((notice, i) => (
            ...
            <div className="mx-auto max-w-4xl">
              ...
              <div className="grid gap-6 md:grid-cols-2">
                {featuredObjects.map((obj, i) => (
                  ...
                  >
                    <Link
                      href={`${getObjectHref(obj.id)}?fromTour=1&tourStep=level3-zone2`}
          ...
          <div className="mx-auto max-w-4xl flex gap-4">
            <Link
              href={`${getZoneHref(secondZone.id)}?fromTour=1&tourStep=level3-zone2`}
              className="flex-1 rounded-xl border border-violet-500/30 bg-violet-500/20 px-8 py-4 text-center font-medium transition hover:bg-violet-500/30"
            >
              Enter This Zone
            </Link>
            <button
