...
export default function Level1Page() {
  ...
  const featuredObjects = getObjectsByLevel("l1").slice(0, 6);
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-6">
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <h2 className="mb-6 text-2xl font-semibold">Level 1 Floor Plan</h2>
          <div className="rounded-xl border border-white/15 bg-white/5 p-8">
            <div className="relative aspect-video bg-gradient-to-br from-amber-950/20 to-black">
              {/* Simplified floor plan visualization */}
              <div className="absolute inset-0 grid grid-cols-2 gap-4 p-4">
                {zones.map((zone) => (
                  <Link
                    key={zone.id}
                    href={`/tour/level-1/zone/${zone.id}`}
                    className="group rounded-lg border border-white/20 bg-white/5 p-4 transition hover:border-amber-400/50 hover:bg-amber-500/10"
                  >
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {featuredObjects.map((obj) => (
              ...
              >
                ...
                <h3 className="mb-2 text-lg font-semibold group-hover:text-amber-400 transition">
                  {obj.title}
                </h3>
                <p className="mb-4 text-sm text-white/70 line-clamp-2">
                  {obj.description}
                </p>
                <div className="text-sm text-amber-400">View Object →</div>
              </Link>
            ))}
          </div>
        </motion.div>
