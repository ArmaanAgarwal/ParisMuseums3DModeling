...
export default function Level3Page() {
  ...
  const zones = getZonesByLevel("l3");
  const featuredObjects = getObjectsByLevel("l3").slice(0, 6);

  if (!level) return null;

  return (
    <div className="min-h-screen bg-black text-white">
      <TourProgressWidget currentStep="level-3" />
      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-6">
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <div className="mb-4 text-sm font-medium uppercase tracking-wider text-violet-400">
            Level 3
          </div>
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <h2 className="mb-6 text-2xl font-semibold">Level 3 Floor Plan</h2>
            ...
            <div className="relative aspect-video bg-gradient-to-br from-violet-950/20 to-black">
              <div className="absolute inset-0 grid grid-cols-2 gap-4 p-4">
                {zones.map((zone) => (
                  <Link
                    key={zone.id}
                    href={`/tour/level-3/zone/${zone.id}`}
                    className="group rounded-lg border border-white/20 bg-white/5 p-4 transition hover:border-violet-400/50 hover:bg-violet-500/10"
                  >
                    <h3 className="mb-2 font-semibold group-hover:text-violet-400 transition">
                      {zone.title}
                    </h3>
            ...
            {zones.slice(0, 4).map((zone) => {
              ...
              return exhibits.slice(0, 1).map((exhibit) => (
                <Link
                  ...
                  className="group rounded-xl border border-white/15 bg-white/5 p-6 transition hover:border-violet-400/50 hover:bg-violet-500/10"
                >
                  <div className="mb-2 text-sm font-medium text-violet-400">
                    {zone.title}
                  </div>
                  <h3 className="mb-2 text-xl font-semibold group-hover:text-violet-400 transition">
                    {exhibit.title}
                  </h3>
                  <p className="mb-4 text-sm text-white/70 line-clamp-2">
                    {exhibit.thesis}
                  </p>
                  <div className="text-sm text-violet-400">View Exhibit →</div>
                </Link>
              ));
            })}
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {featuredObjects.map((obj) => (
              ...
              >
                ...
                <h3 className="mb-2 text-lg font-semibold group-hover:text-violet-400 transition">
                  {obj.title}
                </h3>
                <p className="mb-4 text-sm text-white/70 line-clamp-2">
                  {obj.description}
                </p>
                <div className="text-sm text-violet-400">View Object →</div>
              </Link>
            ))}
          </div>
        </motion.div>
